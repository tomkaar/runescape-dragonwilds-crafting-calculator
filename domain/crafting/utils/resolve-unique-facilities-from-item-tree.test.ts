...
describe("resolveUniqueFacilitiesFromItemTree", () => {
    ...
    it("returns an empty array when no items have facilities", () => {
        const tree = [makeResolvedItem({ facilities: [] }), makeResolvedItem({ facilities: [] })];
    ...
    it("collects facilities from multiple root items", () => {
        ...
        expect(resolveUniqueFacilitiesFromItemTree(tree)).toEqual(["Workbench", "Forge"]);
    });
    ...
    it("collects facilities from nested children", () => {
        ...
        const root = makeResolvedItem({ facilities: ["Workbench"], children: [child] });
        expect(resolveUniqueFacilitiesFromItemTree([root])).toEqual(["Workbench", "Forge"]);
    });

    it("collects facilities from deeply nested grandchildren", () => {
        const grandchild = makeResolvedItem({ facilities: ["Anvil"] });
        const child = makeResolvedItem({ facilities: ["Forge"], children: [grandchild] });
        const root = makeResolvedItem({ facilities: ["Workbench"], children: [child] });
        expect(resolveUniqueFacilitiesFromItemTree([root])).toEqual(["Workbench", "Forge", "Anvil"]);
    });
