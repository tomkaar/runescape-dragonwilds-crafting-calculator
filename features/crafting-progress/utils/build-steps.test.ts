...
import { buildSteps, computeRemainingQuantities, computeCoverageWarnings } from "./build-steps";
...
describe("buildSteps", () => {
  it("includes a marked single-level material with its raw quantity", () => {
    registerItems({
      root: makeItem("root", [makeVariant(makeRecipe(1, [{ itemId: "leaf", quantity: 5 }]))]),
    ...
    const result = buildSteps({
      ...
      allItems: {
        root: [{ id: "m1", itemId: "leaf", quantity: 5, nodeId: "root_leaf", state: "TODO" }],
      },
      multipliers: { root: 1 },
      owned: {},
    });
    ...
    expect(result).toEqual([
      expect.objectContaining({ itemId: "leaf", quantity: 5, depth: 1, hasChildren: false }),
    ]);
  });
  ...
  it("excludes materials marked DONE", () => {
    ...
    const result = buildSteps({
      ...
      allItems: {
        root: [
          { id: "m1", itemId: "leafDone", quantity: 3, nodeId: "root_leafDone", state: "DONE" },
          { id: "m2", itemId: "leafTodo", quantity: 4, nodeId: "root_leafTodo", state: "TODO" },
        ],
      },
      multipliers: { root: 1 },
      owned: {},
  ...
  it("nets a flat single-level material against owned stock", () => {
    registerItems({
      root: makeItem("root", [makeVariant(makeRecipe(1, [{ itemId: "leaf", quantity: 20 }]))]),
      leaf: makeItem("leaf", [makeVariant(null)]),
    });
    ...
    const result = buildSteps({
      filteredItemIds: ["root"],
      allItems: {
        root: [{ id: "m1", itemId: "leaf", quantity: 20, nodeId: "root_leaf", state: "TODO" }],
      },
      multipliers: { root: 1 },
      owned: { leaf: 15 },
    });
  ...
  it("cascades owned stock of an intermediate material down to its own ingredients", () => {
    ...
    registerItems({
      root: makeItem("root", [makeVariant(makeRecipe(1, [{ itemId: "mid", quantity: 48 }]))]),
      mid: makeItem("mid", [makeVariant(makeRecipe(1, [{ itemId: "leaf", quantity: 9 }]))]),
      leaf: makeItem("leaf", [makeVariant(null)]),
    });
    ...
    const result = buildSteps({
      ...
      allItems: {
        root: [
          { id: "m1", itemId: "mid", quantity: 48, nodeId: "root_mid", state: "TODO" },
          { id: "m2", itemId: "leaf", quantity: 432, nodeId: "root_mid_leaf", state: "TODO" },
        ],
      },
      multipliers: { root: 1 },
    ...
    expect(leaf.parents).toEqual([expect.objectContaining({ itemId: "mid", quantity: 23 })]);
  });
  ...
  it("aggregates a shared intermediate and leaf across independent tracked items", () => {
    ...
    registerItems({
      rootA: makeItem("rootA", [makeVariant(makeRecipe(1, [{ itemId: "mid", quantity: 10 }]))]),
      rootB: makeItem("rootB", [makeVariant(makeRecipe(1, [{ itemId: "mid", quantity: 20 }]))]),
      mid: makeItem("mid", [makeVariant(makeRecipe(1, [{ itemId: "leaf", quantity: 9 }]))]),
      leaf: makeItem("leaf", [makeVariant(null)]),
    });
    ...
    const result = buildSteps({
      ...
      allItems: {
        rootA: [
          { id: "a1", itemId: "mid", quantity: 10, nodeId: "rootA_mid", state: "TODO" },
          { id: "a2", itemId: "leaf", quantity: 90, nodeId: "rootA_mid_leaf", state: "TODO" },
        ],
        rootB: [
          { id: "b1", itemId: "mid", quantity: 20, nodeId: "rootB_mid", state: "TODO" },
          { id: "b2", itemId: "leaf", quantity: 180, nodeId: "rootB_mid_leaf", state: "TODO" },
        ],
      },
  ...
  it("sorts deepest materials first, then intermediates before leaves, then by name", () => {
    registerItems({
      ...
      mid: makeItem("mid", [makeVariant(makeRecipe(1, [{ itemId: "zeta", quantity: 1 }]))]),
    ...
    const result = buildSteps({
      ...
      allItems: {
        root: [
          { id: "m1", itemId: "mid", quantity: 1, nodeId: "root_mid", state: "TODO" },
          { id: "m2", itemId: "alpha", quantity: 1, nodeId: "root_alpha", state: "TODO" },
          { id: "m3", itemId: "zeta", quantity: 1, nodeId: "root_mid_zeta", state: "TODO" },
        ],
      },
      multipliers: { root: 1 },
      owned: {},
  ...
  it("rounds a fractional deficit up and flags the material whose ingredient step was never marked", () => {
    ...
    registerItems({
      rootA: makeItem("rootA", [makeVariant(makeRecipe(1, [{ itemId: "mid", quantity: 13 }]))]),
      rootB: makeItem("rootB", [makeVariant(makeRecipe(1, [{ itemId: "mid", quantity: 17 }]))]),
      mid: makeItem("mid", [makeVariant(makeRecipe(1, [{ itemId: "leaf", quantity: 9 }]))]),
      leaf: makeItem("leaf", [makeVariant(null)]),
    });
    ...
    const result = buildSteps({
      filteredItemIds: ["rootA", "rootB"],
      allItems: {
        rootA: [{ id: "a1", itemId: "mid", quantity: 13, nodeId: "rootA_mid", state: "TODO" }],
        rootB: [
          { id: "b1", itemId: "mid", quantity: 17, nodeId: "rootB_mid", state: "TODO" },
          { id: "b2", itemId: "leaf", quantity: 153, nodeId: "rootB_mid_leaf", state: "TODO" },
        ],
      },
      multipliers: { rootA: 1, rootB: 1 },
      owned: { mid: 8 },
...
describe("computeCoverageWarnings", () => {
  function stepEntry(overrides: Partial<StepEntry> & Pick<StepEntry, "itemId" | "quantity">): StepEntry {
    return {
  ...
  it("returns no warnings when every root using the parent also uses the child", () => {
    ...
    const aggregated = new Map([["parent", parent], ["child", child]]);
  ...
  it("flags roots that use the parent but never marked the child", () => {
    ...
    const child = stepEntry({
      itemId: "child",
      quantity: 20,
      parents: [{ itemId: "parent", name: "parent", image: null, quantity: 20 }],
  ...
  it("returns no warnings when the parent is a tracked root, not an aggregated material", () => {
    const child = stepEntry({
      itemId: "child",
      quantity: 5,
      parents: [{ itemId: "someRoot", name: "someRoot", image: null, quantity: 5 }],
...
describe("computeRemainingQuantities", () => {
  function entry(overrides: Partial<StepEntry> & Pick<StepEntry, "itemId" | "quantity">): StepEntry {
    return {
      name: overrides.itemId,
      image: null,
      parents: [],
  ...
  it("returns the gross quantity unchanged when nothing is owned", () => {
    ...
    expect(computeRemainingQuantities(aggregated, {})).toEqual(new Map([["a", 10]]));
  });
  ...
  it("excludes an item fully covered by owned stock", () => {
    ...
    expect(computeRemainingQuantities(aggregated, { a: 10 }).has("a")).toBe(false);
  });

  it("cascades a parent's deficit ratio down to a child with a single parent", () => {
    const aggregated = new Map<string, StepEntry>([
      ["mid", entry({ itemId: "mid", quantity: 48 })],
      ["leaf", entry({ itemId: "leaf", quantity: 432, parents: [{ itemId: "mid", name: "mid", image: null, quantity: 432 }] })],
    ]);
    const remaining = computeRemainingQuantities(aggregated, { mid: 25, leaf: 100 });
