...
function makeSourceItem(itemId: string) {
  return { id: itemId, name: `${itemId}-name`, image: null, variants: [], facilities: [] };
}
...
describe("buildOwnedMaterials", () => {
  it("returns empty array when there are no tracked items", () => {
    expect(buildOwnedMaterials({ trackedItemIds: [], allItems: {}, multipliers: {} })).toEqual([]);
  });

  it("returns empty array when a tracked item has no marked materials", () => {
    mockResolve.mockReturnValue([makeTreeLeaf("iron-ore", 2)]);
    expect(
      buildOwnedMaterials({ trackedItemIds: ["sword"], allItems: {}, multipliers: {} }),
    ).toEqual([]);
  });

  it("skips entries that have no nodeId", () => {
    mockResolve.mockReturnValue([]);
    mockSource.mockReturnValue(makeSourceItem("iron-ore") as ReturnType<typeof sourceItemById>);
    const entry: SelectedMaterial = { id: "e1", itemId: "iron-ore", quantity: 2, state: "TODO" };
  ...
  it("returns one entry for a single marked material", () => {
    ...
    expect(result[0].nodeRefs).toEqual([{ trackedItemId: "sword", nodeId: "iron-ore" }]);
  });
  ...
  it("falls back to entry.quantity when nodeId is not in the resolved tree", () => {
    ...
    mockSource.mockReturnValue(makeSourceItem("wood") as ReturnType<typeof sourceItemById>);
  ...
  it("keeps distinct materials as separate entries", () => {
    ...
    mockSource.mockImplementation((id) => makeSourceItem(id as string) as ReturnType<typeof sourceItemById>);
