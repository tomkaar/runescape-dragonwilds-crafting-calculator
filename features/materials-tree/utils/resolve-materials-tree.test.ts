...
describe("children", () => {
	it("includes child nodes when the item has recipe materials", () => {
		...
		const parent = makeItem("parent-item", [
			makeVariant(makeRecipe(1, [{ itemId: "child-item", quantity: 2 }])),
		]);
	...
	it("sets child nodeId as parentNodeId_childItemId", () => {
		...
		const parent = makeItem("iron-bar", [
			makeVariant(makeRecipe(1, [{ itemId: "coal", quantity: 1 }])),
		]);
		mockSourceItemById.mockImplementation((id) => {
...
describe("multiple variants", () => {
	...
	it("selector node has empty facilities", () => {
		mockSourceItemById.mockReturnValue(
			makeItem("multi-item", [
				makeVariant(makeRecipe(1, [], [Facility[7]])),
				makeVariant(null),
			]),
		);
		const [selector] = resolveMaterialsTree("multi-item");
